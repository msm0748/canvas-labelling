...
export class Rectangle extends Shape {
	...
	updateCornerPoint(cX: number, cY: number) {
	...
	calculateDragOffsets(offsetX: number, offsetY: number, position: RectanglePosition) {
	...
	resizing(offsetX: number, offsetY: number) {
	...
	nearPoint(
		...
		const resizePoint = get(this.resizePoint);
	...
	isPointInside(offsetX: number, offsetY: number) {
		const { x: sX, y: sY } = this.points[0];
		const { x: cX, y: cY } = this.points[1];
		const width = cX - sX;
		const height = cY - sY;
	...
	positionWithinElement(offsetX: number, offsetY: number) {
		const [{ x: sX, y: sY }, { x: cX, y: cY }] = this.points;
